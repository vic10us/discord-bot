...
public class GamesModule : CustomModule<SocketCommandContext>
{
    ...
    public IDiceGameService DiceGameService { get; set; }
    ...
    public async Task RollDice(uint betAmount = 0)
    {
        await _cacheContext.WithLock(async () =>
        {
            ...
            var rolls = DiceGameService.GetNextRolls(6, 2).ToArray();
            ...
            await Task.Delay(1000);
            var opponentRolls = DiceGameService.GetNextRolls(6, 2).ToArray();
